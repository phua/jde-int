...
(defun jde-int-parse-project-file (project-file)
  (when (file-exists-p project-file)
    (let* ((project-description (car (xml-parse-file project-file)))
      ...
      (puthash text (make-hash-table) projects)
...
(defun jde-int-resolve-project-path (project description)
  (mapc #'(lambda (path)
	    (puthash 'sourcepath
		     (union (gethash 'sourcepath (gethash path projects))
			    (gethash 'sourcepath description)
			    :test #'string=)
		     description)
	    (puthash 'global-classpath
		     (union (gethash 'global-classpath (gethash path projects))
			    (gethash 'global-classpath description)
			    :test #'string=)
		     description))
